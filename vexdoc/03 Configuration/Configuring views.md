...
Pick a built-in view explicitly every time: `vex view <focus> <view-name>`, one of the 6 covered in [[Views]]. `config.lua`'s `default.view` key looks like it should let you skip naming a view each time, but nothing in vex currently reads `default.view` (or any other key under `default`) — see the callout on [[Configuring vex (config.lua)]]. There's also no shorthand for "focus only, default view" today: giving `vex view` exactly one positional argument treats it as the *view* name (defaulting the focus to `prev`), not the other way around — see [[CLI reference]].
