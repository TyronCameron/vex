...
> `vex init` creates a `.vex/tasks` folder, described in [[CLI reference]] as where "resolution rules" and custom task types would live — but nothing in vex currently reads from it. The four built-in types (`task`, `abstract`, `decision`, `exploration`, `atom`) are registered directly in `src/core/taskdefinitions.lua`. Tracked as `implement-task-type-loader-1` in the project's own vex tasks.
...
Nothing stops you from adding arbitrary extra fields to any task today (see the "not schema-validated yet" section of [[Frontmatter schema]]) — you just don't get schema validation or defaults for them until this is implemented. Choosing among the 4 built-in types via `--vextype` (see [[Task types]]) is the extent of what's configurable today.
