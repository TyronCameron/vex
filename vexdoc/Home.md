...
- **[[Installing vex]]** — get the `vex` binary running and shell completion set up.
- **[[Creating your first task using vex]]** — `init`, `add`, `show`, `resolve` in five minutes.
- **[[Using a recipe to create a sequence of tasks]]** — stamp out a whole structure of tasks in one command.
- **Features**
  - **[[CLI reference]]** — every command, every flag.
  - **[[Task types]]** — `exploration`, `abstract`, `decision`, `atom`, and how they chain together.
  - **[[Frontmatter schema]]** — every field a task file can carry.
  - **[[Views]]** — tabular, kanban, overview, dependency diagrams, and more.
- **Configuration**
  - **[[Configuring vex (config.lua)]]** — the one config file that's actually loaded today.
  - **[[Configuring event hooks]]**, **[[Configuring focuses]]**, **[[Configuring recipes]]**, **[[Configuring task types]]**, **[[Configuring views]]** — the extension points that are scaffolded but not wired up yet.
...
(All three confirmed working exactly as written.) Then triage without opening a single file:
...
`owner` isn't a specially-validated field yet (see the callout on [[Frontmatter schema]]) — it's a plain frontmatter key like any other, which is exactly why this already works: `vex add --owner bob` just writes `owner: bob` into the task file, and `--filter owner:bob` matches it back out. **The shared git repo is doing the syncing and the notifying; vex is just making sure the data underneath stays structured and diffable.** That's arguably more honest than a tool that promises real-time notifications it doesn't have.

### 3. Spinning up a repeatable project skeleton

Kicking off a new milestone always means the same handful of tasks. Instead of retyping them, wrap them in a recipe once (see [[Configuring recipes]] for where this is headed, and the built-in `abstract` recipe for what exists today):

```txt
vex recipe abstract Launch the next milestone --status todo
```

and get a fully-formed parent task back (vex prints its vexid). Growing it into a full milestone by nesting the rest of the work under its `children` is the intent — not yet the reality, per the warning in example 1 above. As recipes become user-definable (currently code-only — see [[Configuring recipes]]) and the linking bug gets fixed, this is the shape of "one command, whole project scaffold."
